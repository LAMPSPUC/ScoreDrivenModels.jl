...
dates = collect(Date(1961):Month(1):Date(2000, 12))
model = structural(log.(y_train), 12)
ss = statespace(model)
log_sim = simulate(ss, 60, 1000)
exp_sim = exp.(log_sim)[:, 1, :]
quantiles = get_quantiles([0.025, 0.5, 0.975], exp.(log_sim))[: , 1, :]

p2 = plot(dates[401:460], exp_sim, color = "grey", width = 0.05, label = "", ylims = (0, 70))
plot!(p2, dates[360:460], y[360:460], label = "ANE", color = "black", xlabel = "Months", ylabel = "GWmed", legend = :topright)
plot!(p2, dates[401:460], quantiles, label = ["Quantiles" "" ""], color = "red", line = :dash)
