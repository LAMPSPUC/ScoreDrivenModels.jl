@testset "Estimate" begin
    @testset "Beta" begin
        ...
        @testset "Estimation by passing number of seeds" begin
            # LBFGS
            gas = GAS.Model(1, 1, Beta, 0.0)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
            # NelderMead
            gas = GAS.Model(1, 1, Beta, 0.0)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.NelderMead(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
            # IPNewton
            gas = GAS.Model(1, 1, Beta, 0.0)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.IPNewton(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "Estimation by passing seeds" begin
            ...
            fit!(gas, simulation; verbose = 1, opt_method = GAS.LBFGS(gas, seeds))
            test_coefficients_GAS_1_1(gas, ω, A, B)
            # NelderMead
            gas = GAS.Model(1, 1, Beta, 0.0)
            seeds = [[0.1, 0.1, 0.5, 0.5, 0.5, 0.5]]
            fit!(gas, simulation; verbose = 1, opt_method = GAS.NelderMead(gas, seeds))
            test_coefficients_GAS_1_1(gas, ω, A, B)
            # IPNewton
            gas = GAS.Model(1, 1, Beta, 0.0)
            seeds = [[0.1, 0.1, 0.5, 0.5, 0.5, 0.5]]
            fit!(gas, simulation; verbose = 1, opt_method = GAS.IPNewton(gas, seeds))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
    end
    ...
    @testset "Lognormal" begin
        @testset "Scaling = 0.0" begin
            ...
            gas = GAS.Model(1, 1, LogNormal, 0.0)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "Scaling = 0.5" begin
            ...
            gas = GAS.Model(1, 1, LogNormal, 0.5)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "Scaling = 1.0" begin
            ...
            gas = GAS.Model(1, 1, LogNormal, 1.0)
            fit!(gas, simulation; verbose = 1, opt_method = GAS.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "GAS.Model([1, 12], [1, 12])" begin
