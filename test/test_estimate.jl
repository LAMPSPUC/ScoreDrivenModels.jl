@testset "Estimate" begin
    @testset "Beta" begin
        ...
        @testset "Estimation by passing number of seeds" begin
            # LBFGS
            gas = ScoreDrivenModels.Model(1, 1, Beta, 0.0)
    ...
    @testset "Lognormal" begin
        @testset "Scaling = 0.0" begin
            ...
            gas = ScoreDrivenModels.Model(1, 1, LogNormal, 0.0)
            fit!(gas, simulation; verbose = 1, opt_method = ScoreDrivenModels.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "Scaling = 0.5" begin
            ...
            gas = ScoreDrivenModels.Model(1, 1, LogNormal, 0.5)
            fit!(gas, simulation; verbose = 1, opt_method = ScoreDrivenModels.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "Scaling = 1.0" begin
            ...
            gas = ScoreDrivenModels.Model(1, 1, LogNormal, 1.0)
            fit!(gas, simulation; verbose = 1, opt_method = ScoreDrivenModels.LBFGS(gas, 3))
            test_coefficients_GAS_1_1(gas, ω, A, B)
        end
        @testset "ScoreDrivenModels.Model([1, 12], [1, 12])" begin
