...
# Links
function link(::Type{Beta}, param::Matrix{T}, t::Int) where T 
    return [
        link(LogLink, param[t, 1], zero(T));
        link(LogLink, param[t, 2], zero(T))
    ]
end
function unlink(::Type{Beta}, param_tilde::Matrix{T}, t::Int) where T 
    return [
        unlink(LogLink, param_tilde[t, 1], zero(T));
...
function update_dist(::Type{Beta}, param::Matrix{T}, t::Int) where T
    small_threshold!(param, T(1e-8), t)
